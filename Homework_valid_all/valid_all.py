...
def valid_all(input_validation: Callable, result_validation: Callable,
              on_fail_repeat_times=1, default_behavior=None) -> Any:
    ...
    def decorate(func: Callable) -> Any:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            if not valid:
                raise InputParameterVerificationError("Not valid input param")
            else:
...
def json_validation(new_dict: dict) -> Any:
    ...
    with open('wrong_scheme.txt') as scheme:
        parsed_scheme = json.load(scheme)
    try:
        jsonschema.validate(new_dict, parsed_scheme)
        print(new_dict)
        return new_dict
...
@valid_all(input_validation=regex_validation,
           result_validation=json_validation,
           on_fail_repeat_times=-5,
           default_behavior=None)
def main_function(random_str: str) -> dict:
    """Main Function."""
    if random_str:
        json = {"Key": random_str}
        return json
